        ...
        for data in [self.train, self.valid, self.test]:
            if isinstance(data, list):
                for sub_data in data:
                    ...
                    sub_data[DEFAULT_USER_COL] = sub_data[DEFAULT_USER_COL].apply(
                        lambda x: self.user2id[x]
                    )
                    sub_data[DEFAULT_ITEM_COL] = sub_data[DEFAULT_ITEM_COL].apply(
                ...
                # Map user_idx and item_idx
                data[DEFAULT_USER_COL] = data[DEFAULT_USER_COL].apply(
                    lambda x: self.user2id[x]
                )
                data[DEFAULT_ITEM_COL] = data[DEFAULT_ITEM_COL].apply(
                    lambda x: self.item2id[x]
                )
    ...
    def _intersection(self):
        ...
        for data in [self.valid, self.test]:
            if isinstance(data, list):
                for sub_data in data:
                    sub_data = sub_data[
                        sub_data[DEFAULT_USER_COL].isin(user_pool)
                        & sub_data[DEFAULT_ITEM_COL].isin(items_pool)
                    ]
            else:
                data = data[
                    data[DEFAULT_USER_COL].isin(user_pool)
