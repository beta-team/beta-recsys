...
class TiSASRec(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.time_matrix_K_emb = torch.nn.Embedding(self.time_span + 1, self.hidden_units)
        self.time_matrix_V_emb = torch.nn.Embedding(self.time_span + 1, self.hidden_units)
    ...
    def forward(self, user_ids, log_seqs, time_matrices, pos_seqs, neg_seqs):  # for training
        ...
        """
        log_feats = self.seq2feats(user_ids, log_seqs, time_matrices)  # user_ids hasn't been used yet
...
class TiSASRecEngine(ModelEngine):
    ...
    def train_an_epoch(self, sampler, epoch_id):
        ...
        for batch_id in range(self.num_batch):
            ...
            batch_data = np.array(u), np.array(seq), np.array (time_seq), np.array (time_matrix), np.array(pos), np.array(neg)
