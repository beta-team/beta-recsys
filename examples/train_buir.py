...
from beta_rec.models.buir import BUIREngine
...
def parse_args():
    ...
    Returns:
        args object.
    """
    parser = argparse.ArgumentParser(description="Run BUIR..")
    parser.add_argument(
        "--config_file",
        nargs="?",
        type=str,
        default="../configs/buir_default.json",
...
class BUIR_train(TrainEngine):
    """An instance class from the TrainEngine base class."""

    def __init__(self, config):
        """Initialize BUIR_train Class.

        Args:
            config (dict): All the parameters for the model.
        """
        self.config = config
        super(BUIR_train, self).__init__(config)
        self.load_dataset()
        self.build_data_loader()
        self.engine = BUIREngine(self.config)
...
def tune_train(config):
    ...
    Args:
        config (dict): All the parameters for the model.
    """
    train_engine = BUIR_train(DictToObject(config))
