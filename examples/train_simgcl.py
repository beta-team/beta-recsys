...
from beta_rec.models.simgcl import SimGCLEngine
...
def parse_args():
    ...
    Returns:
        args object.
    """
    parser = argparse.ArgumentParser(description="Run SimGCL..")
    parser.add_argument(
        "--config_file",
        nargs="?",
        type=str,
        default="../configs/simgcl_default.json",
...
class SimGCL_train(TrainEngine):
    """An instance class from the TrainEngine base class."""

    def __init__(self, config):
        """Initialize SimGCL_train Class.

        Args:
            config (dict): All the parameters for the model.
        """
        self.config = config
        super(SimGCL_train, self).__init__(config)
        self.load_dataset()
    ...
    def train(self):
        ...
        self.engine = SimGCLEngine(self.config)
...
def tune_train(config):
    ...
    Args:
        config (dict): All the parameters for the model.
    """
    train_engine = SimGCL_train(DictToObject(config))
...
if __name__ == "__main__":
    args = parse_args()
    train_engine = SimGCL_train(args)
    if args.tune:
